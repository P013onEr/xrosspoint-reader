...
int titleFontId() { return UI_12_FONT_ID; }     // Requested main title size: 12px
int subtitleFontId() { return SMALL_FONT_ID; }  // Requested subtitle size: 8px
int guideFontId() { return SMALL_FONT_ID; }     // Closest available to requested 6px
...
void RoundedRaffTheme::drawButtonMenu(GfxRenderer& renderer, Rect rect, int buttonCount, int selectedIndex,
  ...
  for (int i = pageStartIndex; i < buttonCount && i < pageStartIndex + pageItems; ++i) {
    ...
    const int rowWidth = std::min(
        menuMaxWidth, renderer.getTextWidth(titleFontId(), truncatedLabel.c_str(), EpdFontFamily::BOLD) + kRowPaddingX);
...
void RoundedRaffTheme::drawList(const GfxRenderer& renderer, Rect rect, int itemCount, int selectedIndex,
  ...
  for (int i = pageStartIndex; i < itemCount && i < pageStartIndex + pageItems; i++) {
      ...
      if (!valueText.empty()) {
        ...
        if (maxValueWidth > 0) {
          const std::string truncatedValue =
              renderer.truncatedText(titleFontId(), valueText.c_str(), maxValueWidth, EpdFontFamily::REGULAR);
          const int valueW = renderer.getTextWidth(titleFontId(), truncatedValue.c_str(), EpdFontFamily::REGULAR);
          renderer.drawText(titleFontId(), rowX + rowWidth - kInteractiveInsetX - valueW,
                            rowY + (rowHeight - renderer.getLineHeight(titleFontId())) / 2, truncatedValue.c_str(),
    ...
    if (hasSubtitle) {
      ...
      auto title = renderer.truncatedText(titleFontId(), rowTitle(i).c_str(), textAreaWidth, EpdFontFamily::BOLD);
      ...
      if (subtitleRaw.empty()) {
        ...
        renderer.drawText(titleFontId(), rowX + kInteractiveInsetX, centeredTitleY, title.c_str(), !isSelected,
                          EpdFontFamily::BOLD);
      } else {
        const int titleY = rowY + subtitleTopPadding;
        const int subtitleY = titleY + titleLineHeight + subtitleInterLineGap;
        auto subtitle =
            renderer.truncatedText(subtitleFontId(), subtitleRaw.c_str(), textAreaWidth, EpdFontFamily::REGULAR);
        renderer.drawText(titleFontId(), rowX + kInteractiveInsetX, titleY, title.c_str(), !isSelected,
                          EpdFontFamily::BOLD);
      ...
      auto title = renderer.truncatedText(titleFontId(), rowTitle(i).c_str(), textAreaWidth, EpdFontFamily::BOLD);
      renderer.drawText(titleFontId(), rowX + kInteractiveInsetX,
                        rowY + (rowHeight - renderer.getLineHeight(titleFontId())) / 2, title.c_str(), !isSelected,
                        EpdFontFamily::BOLD);
    }
  }
